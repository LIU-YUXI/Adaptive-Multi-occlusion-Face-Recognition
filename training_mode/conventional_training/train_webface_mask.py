...
def train_one_epoch(data_loader,  backbone_student, header, 
...
def train(conf):
    ...
    '''
    # load student model
    backbone_student = BackboneFactory(conf.backbone_type, conf.backbone_conf_file)# iresnet100(num_features=conf.embedding_size)
    ...
    for epoch in range(ori_epoch, conf.epoches):
        train_one_epoch(data_loader, model,header,
                        model_optimizer, header_optimizer, 
