...
sys.path.append('..')
from data_processor.test_dataset import CommonTestDataset96
...
import backbone.net_sia as net_sia
...
class FaceModel(torch.nn.Module):
    ...
    def forward(self, data, data_clip):
        # pred = F.normalize(self.pre_model(data))
        f_masked, focc_masked, output, output_occ, f_diff, out = self.backbone.forward(data,data)
        return output
...
if __name__ == '__main__':
    ...
    # define dataloader
    data_loader = DataLoader(CommonTestDataset96(cropped_face_folder, image_list_file_path, False, preprocess), 
    ...
    model_loader = net_sia.LResNet50E_IR_Sia(is_gray=False)
    # pre_model_loader = iresnet100(num_features=args.embedding_size)# ModelLoader(backbone_factory)
    ...
    if os.path.isdir(args.model_path):
        ...
        # pre_model = InceptionResnetV1(pretrained='casia-webface').cuda(args.device).eval()
        model_loader.load_state_dict(torch.load(args.model_path,map_location='cpu')['model_state_dict'])# ['state_dict']
        model = model_loader.cuda(args.device)
